...
create table public.expansion_areas (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles(id) on delete cascade,
  level_required integer not null check (level_required > 0),
  name text not null,
  owned boolean not null default false,
  fog_state text not null default 'hidden' check (fog_state in ('hidden', 'revealing', 'revealed')),
  rewards text[] not null default '{}',
  animation_state text not null default 'idle' check (animation_state in ('idle', 'queued', 'playing', 'complete')),
...
create table public.seasonal_events (
  ...
  ambient_effects text[] not null default '{}'
);

alter table public.sanctuary_discoveries
  add column if not exists premium_only boolean not null default true,
  add column if not exists artwork_ref text,
...
alter table public.expansion_areas enable row level security;
alter table public.app_notifications enable row level security;
alter table public.notification_preferences enable row level security;
alter table public.seasonal_events enable row level security;

create policy "expansion areas owned by user" on public.expansion_areas for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "notifications owned by user" on public.app_notifications for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "notification preferences owned by user" on public.notification_preferences for all using (auth.uid() = user_id) with check (auth.uid() = user_id);
create policy "seasonal events are readable" on public.seasonal_events for select using (true);
